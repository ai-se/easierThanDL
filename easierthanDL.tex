...
%%% The following is specific to ESEC/FSE'17 and the paper
%%% 'Easy over Hard: A Case Study on Deep Learning'
%%% by Wei Fu and Tim Menzies.
%%%
\setcopyright{acmcopyright}
\acmDOI{10.1145/3106237.3106256}
\acmISBN{978-1-4503-5105-8/17/09}
...
<concept_desc>Software and its engineering~Search-based software
...
\ccsdesc[500]{Software and its engineering~Collaboration in software
development}
...
{\it Accuracy} is defined as the number of  correctly
...
\begin{figure}[!htp]
    \centering
     \includegraphics[width=0.47\textwidth,height=2.2in]{pic/OurSVM-Xu'sSVM.pdf} % THIS IS FOR SINGLE COLUM
...
use the SVM learner from scikit-learn with the setting $\gamma = \frac{1}{200}$ and $\mathit{kernel=}$``rbf'', which are used by XU. After that, the same training and testing knowledge unit pairs are applied to SVM.
 ...
 \begin{figure}[!htp]
    \centering
    \includegraphics[width=0.47\textwidth,height=2.2in]{pic/TunedSVM-CNN.pdf} % THIS IS FOR SINGLE COLUM
...
\textbf{RQ2: Can DE tune a standard learner such that it outperforms XU's deep learning method?}
 ...
 \begin{figure}[!t]
    \centering
%    \includegraphics[width=0.48\textwidth,height=2.7in]{pic/workflow.pdf} % THIS IS FOR SINGLE COLUM
     \includegraphics[width=0.47\textwidth,height=2.2in]{pic/TunedSVM-SVM.pdf} % THIS IS FOR SINGLE COLUM
    \caption{Score delta between the tuned SVM and XU's baseline SVM in terms of precision, recall and F1-score. Positive values mean
            the tuned SVM is better than XU's SVM in terms of different measures; Otherwise, XU's SVM is better.}
...
\begin{table}[!htp]
\centering
\caption{Comparison of the tuned SVM with XU's CNN method. The best scores are marked in \textbf{bold}. }
\resizebox{0.47\textwidth}{!}{
\begin{tabular} {@{}l l  c c  c c c@{}}
\hline
   \multirow{2}{*}{Metrics} &  \multirow{2}{*}{Methods} &  \multirow{2}{*}{Duplicate} &  
...
the deep learning method, CNN, does not have any performance advantage over our tuning approach.
 \end{myshadowbox}
  ...
  \begin{figure}[!t]
    \centering
%    \includegraphics[width=0.48\textwidth,height=2.7in]{pic/workflow.pdf} % THIS IS FOR SINGLE COLUM
     \includegraphics[width=0.47\textwidth,height=2.1in]{pic/TunedSVM-OurSVM.pdf} % THIS IS FOR SINGLE COLUM
    \caption{Score delta between tuned SVM and Our Untuned SVM in terms of precision, recall and F1-score. Positive values mean
            the tuned SVM is better than our untuned SVM in terms of different measures; Otherwise,  our  SVM is better.}
...
the baseline method in this study. As shown in RQ1, our implementation similar results to XU's on the same data sets. Hence, we believe that our implementation reflect the original
...
method sometimes produced {\em better} results than classic deep learning and (b)~our DE runs 84 times faster (i.e. much faster than those reported in~\cite{spring2016scalable}).

That said, this new fast deep learning method deserves our close attention since, using it, we conjecture that our DE tools
