...
\item Most paper's baseline methods are either not publicly available or too complex to implement~\cite{white2016deep,lam2015combining}. XU define their baseline methods precisely enough so others can confidently reproduce it locally.
...
training and testing data are public available, which can be used for our proposed method. Since the same training and testing data are used for XU's CNN and our proposed method,  we can compare results from our method to their CNN results.
...
each time one fold with $640$ knowledge units pairs is used as the tuning data, and the remaining folds with $5,760$
knowledge units are used as  the new training data, then the output SVM model will be evaluated on the testing data. Since we notice that there is some variance in the performance of different word2vec models trained by randomly selected 100,000 knowledge units, we generate 10 different word2vec models by using different random seeds, each of which will be used for a {\it 10-fold cross-validation}. Therefore,
all the performance scores reported below are median values over 100 runs.
...
knowledge unit pairs correctly classified as $C_j$ over the number of knowledge unit pairs classified as $C_j$ and
...
\bi
\item
Compare   XU's Word Embedding + SVM  with our implementation;
\item
...
\begin{figure}[!htp]
    \centering
     \includegraphics[width=0.49\textwidth,height=4in]{pic/OurSVM-Xu'sSVM.pdf} % THIS IS FOR SINGLE COLUM
...
% %%%%%%%%%%%%%%%% XU-Wei-baseline-old end%%%%%%%%%%%%%%%%%%%%%


%%%%%%%%%%%%%%%% XU-Wei-baseline begin%%%%%%%%%%%%%%%%%%%%%
...
\hline
   ...
   \multirow{2}{*}{Isolated} &  \multirow{2}{*}{Overall} \\ \\ \hline
   \multirow{2}{*}{Precision}
    & Our SVM &\textbf{0.651} &0.471 & 0.749 &0.390& 0.566 \\
   & XU's SVM &0.611 &\textbf{0.560} &\textbf{0.787}&\textbf{0.676}&\textbf{0.659} \\ \hline
   \multirow{2}{*}{Recall} 
   & Our SVM & 0.446& 0.286 & 0.629 & \textbf{0.736}  & 0.524 \\ 
   & XU's SVM  & \textbf{0.725} &\textbf{0.433}    &\textbf{0.980}  & 0.538 &\textbf{0.669}  \\ \hline
   \multirow{2}{*}{F1-score}
   & Our SVM & 0.529 &  0.357 &0.684  &  0.510&0.520 \\ 
   & XU's SVM & \textbf{0.663} &  \textbf{ 0.488}  & \textbf{0.873} &  \textbf{0.600} &\textbf{0.656} \\ \hline
  \multirow{2}{*}{Accuracy} 
 &  Our SVM&0.445  &  0.286 & 0.628 & 0.735 &0.524 \\
  & XU's SVM & - &  -  &- &  - &\textbf{0.669} \\ \hline
 \end{tabular}}
\label{tab:baseline}
\end{table}
...
use the SVM learner from scikit-learn setting $\gamma = \frac{1}{200}$ and $\mathit{kernel}=$``rbf''. After that, the same training and testing knowledge unit pairs are applied to SVM.
 ...
 our Word Embedding + SVM method has  very  similar or worse performance scores than the baseline method
 reported by XU in ~\cite{xu2016predicting}, with the maximum difference is $0.374$ for Isolated class in  terms of precision.  
%  Except that we have higher precision for Duplicate class~(i.e., $0.651$ v.s. $0.611$) and higher recall for Isolated class(i.e., $0.736$ v.s. $0.538$)
...
% has a higher precision~(i.e., $0.724$ v.s. $0.611$) but a lower  recall (i.e., $0.525$ v.s.$0.725$). 
 ...
 \begin{figure}[!htp]
    \centering
    \includegraphics[width=0.49\textwidth,height=4in]{pic/TunedSVM-CNN.pdf} % THIS IS FOR SINGLE COLUM
...
\hline
 ...
 % & Our SVM &0.736 &0.521 & 0.793 &0.606& 0.664 \\
   ...
   & Tuned SVM&\textbf{0.907} & \textbf{0.867}&\textbf{0.943} &\textbf{0.919} &\textbf{0.909}\\ \hline
   \multirow{3}{*}{Recall} 
   %& Our SVM & 0.550& 0.502 & 0.970 & 0.645  & 0.667 \\ 
   & XU's SVM& 0.725 &0.433    &0.980  & 0.538 &0.669  \\
   & XU's CNN& \textbf{0.898}&\textbf{0.903}    &0.773  & 0.793 &0.842  \\ 
   & Tuned SVM& 0.864 &0.860    &\textbf{0.994}  & \textbf{0.925} &\textbf{0.911}  \\  \hline
   \multirow{3}{*}{F1-score}
   %&Our SVM & 0.629 &  0.511 &0.873  &  0.625&0.660 \\ 
   & XU's SVM& 0.663 &  0.488  & 0.873 &  0.600 &0.656 \\ 
   & XU's CNN& \textbf{0.898} &  0.824  & 0.805 &  0.849 &0.841 \\
   & Tuned SVM& 0.885 & \textbf{ 0.862}  &\textbf{ 0.968} &  \textbf{0.922} &\textbf{0.909} \\\hline
%   \multirow{2}{*}{Accuracy} &  Our SVM&0.550  &  0.402 & 0.970 & 0.645 &0.667 \\
 %  & Xu's SVM& - &  -  &- &  - &0.669 \\ \hline
 \end{tabular}}
\label{tab:RQ2}
...
of the score delta ($\frac{10}{12}$) are negative, which means  that our
implementation is worse than XU's implementation and we do not have any advantage in our baseline method implementation .
...
has  similar or worse performance in all the evaluated metrics 
...
we can  use the performance of our  reimplementation as  a valid lower bound surrogate for
the work of XU.
