...
like word2vec~\cite{mikolov2013distributed}, GloVe ~\cite{pennington2014glove}
...
neural networks. The Skip-gram model learns vector representations of words
 that are useful for predicting the surrounding words in a context window. 
 Given a sentence of words $w_1$,$w_2$,...,$w_T$, the objective of Skip-gram model is to maximize the
...
To improve the computation efficiency, Mikolove et al. \cite{mikolov2013distributed} proposed hierachical softmax and negative sampling
techniques. More details can be found in ~\cite{mikolov2013distributed}.

For each knowledge unit, we firstly clean the data by removing the unnecessary HTML tags and keep short code snippets in
{\it code} tag as Xue et al. Then, for each word $w_i$ in the post processed knowledge unit(including title, question and answers),
we query the trained word2vec model to get the corresponding word vector representation $v_i$. Then the whole knowledge unit
will be converted to vector representation, $Uv = v_i \oplus v_2 \oplus...\oplus v_k $. This vector representation will be used
as the input to SVM.
